# -*- coding: utf-8 -*-
from app.shop_config import get_shop_info
from app.keywords_data import has_tag_friend_invite, has_facebook_tags

def normalize_label(x):
    return str(x).strip().lower() if x is not None else ""

def choose_segment(total_messages=0, negative_count=0, complaint_count=0, inactive_days=0):
    total_messages = int(total_messages or 0)
    negative_count = int(negative_count or 0)
    complaint_count = int(complaint_count or 0)
    inactive_days = int(inactive_days or 0)

    if inactive_days >= 30:
        return "Lost Customer"
    if negative_count >= 3 or complaint_count >= 3:
        return "At-Risk"
    if total_messages >= 20 and negative_count <= 2:
        return "VIP"
    if total_messages >= 5:
        return "Active"
    return "Regular"

# ---------------------------------------------------------------------------
# FIX: เดิม choose_action เช็คคำอย่าง "มีแมลง", "สกปรก", "พนักงานหยาบคาย" กับตัวแปร
# `category` ซึ่งเป็นแค่ "ป้ายชื่อหมวด" (เช่น "ร้องเรียนคุณภาพอาหาร") ที่ไม่เคยมีคำพวกนี้
# อยู่ในป้ายชื่อเลย ทำให้ branch escalate พิเศษหลายจุดเป็น dead code ไม่ทำงานจริง
# ตอนนี้แก้ให้เช็คกับ `message` (ข้อความจริงของลูกค้า) แทน
# ---------------------------------------------------------------------------
def choose_action(sentiment, category, segment, message="", message_tags=None):
    s = normalize_label(sentiment)
    c = str(category or "")
    seg = str(segment or "")
    m = str(message or "")

    # ---------- 1) เคสร้ายแรง/ร้องเรียนหนัก (ตรวจก่อนเสมอ, เช็คกับข้อความจริง) ----------
    # FIX (เคสร้ายแรงที่สุดจากการ audit หน้าเพจจริง: "เจอเส้นผมในอาหารค่ะ ขอให้
    # ตรวจสอบด้วย" เดิมไม่มี "เส้นผม" อยู่ใน list นี้เลย ทำให้ไม่เคยเข้า branch escalate
    # พิเศษนี้แม้ sentiment จะถูกแก้ให้เป็น negative แล้วก็ตาม (ดู TASTE_NEG ใน
    # keywords_data.py ที่เพิ่ม "เส้นผม" เข้าไปด้วยเพื่อให้ keyword ตัดสิน sentiment
    # เป็น negative ได้เองโดยไม่ต้องพึ่ง ML) เพิ่มไว้ตรงนี้เพื่อให้ตอบ apology_quality_
    # escalate (escalate ทุกเซกเมนต์) แทน apology_escalate ธรรมดา เพราะสิ่งแปลกปลอม
    # ในอาหารเป็นเรื่องร้ายแรงกว่าคำร้องเรียนทั่วไปมาก
    if s == "negative" and any(w in m for w in ["แมลง", "สิ่งแปลกปลอม", "อาหารเป็นพิษ", "บูด", "เสียแล้ว", "หมดสภาพ", "เส้นผม"]):
        return "apology_quality_escalate"          # คุณภาพอาหาร - ร้องเรียนหนัก, escalate ทุกเซกเมนต์

    if s == "negative" and (any(w in m for w in ["พนักงานหยาบคาย", "บริการแย่", "ไม่สุภาพ", "ไม่ให้เกียรติ"]) or "ร้องเรียนการบริการ" in c):
        return "apology_staff_escalate"            # ร้องเรียนพนักงาน

    if s == "negative" and (any(w in m for w in ["สกปรก", "กลิ่นเหม็น", "แมลงสาบ", "เลอะเทอะ"]) or "ความสะอาด" in c):
        return "apology_cleanliness_escalate"      # ความสะอาด

    if s == "negative" and (any(w in m for w in ["แอร์", "ร้อนมาก", "เย็นเกินไป", "ร้อนอบอ้าว"]) or "อุณหภูมิ" in c or "แอร์" in c):
        return "apology_temperature_escalate"

    if s == "negative" and ("ห้องน้ำ" in m or "ห้องน้ำ" in c):
        return "apology_restroom_escalate"

    if s == "negative" and (("บิล" in m or "ยอดเงิน" in m or "ยอดไม่ตรง" in m) and ("ผิด" in m or "ไม่ตรง" in m or "เกิน" in m)):
        return "apology_billing_escalate"

    if s == "negative" and (any(w in m for w in ["แอพ", "ระบบ", "เว็บค้าง"]) or "ระบบ" in c):
        return "apology_system_issue"

    if s == "negative" and (any(w in m for w in ["บัตรสมาชิก", "แต้ม"]) or "บัตรสมาชิก" in c):
        return "apology_membership_issue"

    # ---------- 2) จัดส่ง / ออเดอร์ผิดพลาด ----------
    if s == "negative" and (any(w in m for w in ["ส่งช้า", "มาช้า", "รอของนาน", "อาหารมาไม่ครบ"]) or "การจัดส่ง" in c or "Delivery" in c):
        if seg in ["VIP", "At-Risk"]:
            return "apology_coupon_escalate"
        return "apology_check_order"

    if s == "negative" and any(w in m for w in ["ไม่ครบ", "ขาด", "ผิดออเดอร์", "สั่งผิด", "ส่งผิด"]):
        if seg in ["VIP", "At-Risk"]:
            return "apology_coupon_escalate"
        return "apology_wrong_order"

    # ---------------------------------------------------------------------
    # FIX (เคสจริง "โปรลดราคานี้ใช้ได้กับเดลิเวอรี่ด้วยไหมคะ" ถูกตอบ general_support
    # ทั้งที่ category ตัดสินถูกแล้วว่าเป็น "การจัดส่ง (Delivery)"): เดิม category นี้
    # มีแค่ branch ร้องเรียน (ด้านบน, เช็คเฉพาะตอน s=="negative") ไม่มี branch ตอบ
    # ข้อมูลทั่วไปสำหรับคำถามที่เป็นกลางเกี่ยวกับเดลิเวอรี่เลย (ต่างจากหมวดอื่นๆ เช่น
    # "เมนู"/"โปรโมชั่น" ที่มี branch ข้อมูลทั่วไปของตัวเองในส่วนที่ 4) ทำให้ตกไปจบที่
    # general_support เสมอเมื่อไม่ใช่การร้องเรียน เพิ่ม branch นี้ไว้ตรงนี้ (หลัง branch
    # ร้องเรียนข้างบนเสมอ เพื่อให้คำร้องเรียนจริงยังคงได้รับการ apology ก่อน ไม่ถูกบัง)
    #
    # จำกัดเฉพาะ s=="neutral" เท่านั้น (ไม่ใช่ s!="negative" ตามที่ลองตอนแรก) เพราะ
    # เทสกับ regression suite เดิมพบว่า "แพ็คมาดีมาก ของไม่หกเลย" (คำชมเดลิเวอรี่ที่
    # เป็น positive อยู่แล้ว) จะโดน branch นี้แย่งไปตอบ "delivery_info" แทน "thank_you"
    # ที่ถูกต้องอยู่แล้ว ถ้าเช็คแค่ s!="negative" — คำชมเดลิเวอรี่ยังต้องได้ thank_you
    # เหมือนเดิมทุกประการ (ดู "# ---------- 6) positive ทั่วไป ----------" ด้านล่าง)
    # ---------------------------------------------------------------------
    if s == "neutral" and ("การจัดส่ง" in c or "Delivery" in c):
        return "delivery_info"

    # ---------- 3) ทั่วไป negative ----------
    if s == "negative":
        return "apology_escalate"

    # ---------------------------------------------------------------------
    # FIX (ข้อความจริง "ขอชมว่าน้ำแข็งเย็นทุกก้อน" ถูกตอบเป็นข้อมูลที่ตั้งร้าน):
    # category_model (ML) มี class ดิบที่ขึ้นต้นด้วย "ชม..." อยู่ 3 ตัว คือ
    # "ชมรสชาติอาหาร", "ชมการบริการ", "ชมสถานที่/บรรยากาศ" (ดู models/category_model.joblib)
    # ปัญหาคือ "ชมสถานที่/บรรยากาศ" ดันมีคำว่า "สถานที่" อยู่ในตัวเอง ซึ่งไปชนกับกฎข้อ 4
    # ด้านล่างที่เช็ค "สถานที่" in c -> send_location (กฎนั้นตั้งใจไว้จับ category
    # "ที่ตั้งร้าน (Location)" ของลูกค้าที่ถาม ไม่ใช่ category คำชมบรรยากาศ) ทำให้คำชม
    # กลายเป็นถูกส่งพิกัดร้านให้แทนคำขอบคุณ จึงกันไว้ก่อนตรงนี้: ถ้า sentiment เป็น
    # positive และ category ขึ้นต้นด้วย "ชม" (ไม่ว่าจะเป็น label จาก ML หรือจาก
    # keyword เอง เช่น "ชมบรรยากาศร้าน"/"ชมพนักงาน"/"ชมความสะอาด") ให้ตอบขอบคุณทันที
    # ก่อนไปถึงกฎ "สอบถามข้อมูลทั่วไป" ด้านล่าง ไม่ต้องรอไปชนคำบังเอิญ
    # ---------------------------------------------------------------------
    if s == "positive" and c.startswith("ชม"):
        return "thank_you"

    # ---------- 4) สอบถามข้อมูลทั่วไป ----------
    if "โปรโมชั่น" in c or "Promotion" in c or "โปร" in c:
        return "send_promotion"

    if "เมนู" in c or "Menu" in c:
        return "send_menu"

    if "จอง" in c or "Reservation" in c:
        return "reservation_support"

    if "เวลาเปิด" in c or "เวลาทำการ" in c:
        return "send_hours"

    if "แผนที่" in c or "สถานที่" in c or "location" in c or "ที่ตั้ง" in c:
        return "send_location"

    # FIX (เคสจริง "โดยรวมโอเค แต่ราคาแพงขึ้นเยอะเลย" ถูกตอบ "รบกวนแจ้งเมนูที่สนใจ
    # ทางร้านจะแจ้งราคาที่แน่นอนให้ค่ะ" ทั้งที่ลูกค้ากำลังบ่นเรื่องราคาขึ้น ไม่ได้ถามราคา):
    # ต้องเช็ค "ร้องเรียนราคา" (คำร้องเรียน) แยกจาก "ราคา" ทั่วไป (คำถามราคาเฉยๆ) ก่อน
    # เพราะ "ร้องเรียนราคา" ก็มีคำว่า "ราคา" เป็น substring อยู่ในตัวเองด้วย ถ้าไม่แยก
    # เช็คก่อน จะโดน branch "ราคา" in c ด้านล่างตัดหน้าเสมอ (sentiment เคสนี้มักออกมา
    # เป็น "neutral" จาก majority-vote แบบก้ำกึ่ง ไม่ใช่ "negative" ล้วนๆ เพราะมักมีคำชม
    # ปนอยู่ด้วยเช่น "โดยรวมโอเค" จึงไม่เข้า branch ร้องเรียนหนักด้านบนที่ต้อง s=="negative")
    if "ร้องเรียนราคา" in c:
        return "price_feedback_ack"

    if "ราคา" in c:
        return "send_price_info"

    if "ชำระเงิน" in c or "payment" in c:
        return "send_payment_info"

    if "แพ้" in c or "มังสวิรัติ" in c or "ฮาลาล" in c or "ข้อจำกัดด้านอาหาร" in c:
        return "allergy_info_caution"

    if "ใบกำกับภาษี" in c or "ออกบิล" in c:
        return "send_tax_invoice_info"

    if "สมาชิก" in c or "สะสมแต้ม" in c:
        return "send_membership_info"

    if "จัดเลี้ยง" in c or "อีเวนต์" in c or "catering" in c:
        return "catering_support"

    if "สมัครงาน" in c or "รับสมัคร" in c:
        return "job_application_info"

    if "แฟรนไชส์" in c or "franchise" in c:
        return "franchise_info"

    if "ช่องทางติดต่อ" in c:
        return "contact_info"

    if "ซื้อกลับบ้าน" in c or "Takeaway" in c:
        return "takeaway_info"

    if "สาขา" in c or "ทำเล" in c or "อยู่ตรงไหน" in c or "BTS" in c or "MRT" in c:
        return "branch_location_info"

    if "ที่จอดรถ" in c or "จอดรถ" in c or "ลานจอด" in c:
        return "parking_info"

    if "คิวรอโต๊ะ" in c:
        return "waitlist_info"

    if "WiFi" in c or "ไวไฟ" in c:
        return "wifi_info"

    if "สัตว์เลี้ยง" in c:
        return "pet_friendly_info"

    if "เด็ก" in c:
        return "kids_facility_info"

    if "ห้องส่วนตัว" in c or "VIP" in c:
        return "private_room_info"

    if "Corkage" in c:
        return "corkage_info"

    if "ปรับระดับความเผ็ด" in c:
        return "spice_customization_info"

    if "บุฟเฟ่ต์" in c:
        return "buffet_info"

    if "ข้อเสนอแนะ" in c:
        return "feedback_thank_you"

    if "เครื่องดื่มแอลกอฮอล์" in c:
        return "alcohol_menu_info"

    if "ดนตรีสด" in c or "กิจกรรม" in c:
        return "event_info"

    if "พื้นที่จัดส่ง" in c:
        return "delivery_area_info"

    if "ยอดสั่งขั้นต่ำ" in c:
        return "min_order_info"

    if "คืนเงิน" in c or "ยกเลิก" in c or "refund" in c:
        if seg in ["VIP", "At-Risk"]:
            return "refund_priority_support"
        return "refund_cancel_support"

    # ---------------------------------------------------------------------
    # FIX (พบจากคอมเมนต์จริงบน Facebook: "สนุกดีจัง จริงป่ะ" ถูกตอบเป็น
    # "ขออภัยอย่างสูงค่ะ ทางร้านรับทราบปัญหาแล้ว..." ทั้งที่เป็นแค่คำถามกึ่งประชด
    # ไม่ใช่การร้องเรียน): เดิม branch นี้เช็คแค่ "thank_you ถ้า positive ไม่งั้น
    # apology_escalate เสมอ" เป็น ternary 2 ทาง ซึ่งไม่ได้คำนึงว่า sentiment มี 3
    # ค่า (positive/negative/neutral) ทำให้ "neutral" (เช่น ข้อความคำถามล้วนๆ ที่
    # keyword_sentiment_override ตัดสินไว้แล้วว่าไม่ใช่ทั้งคำชมและคำต่อว่า) ถูกเหมา
    # รวมไปกับ "negative" กลายเป็นขอโทษ/escalate ทั้งที่ไม่มีอะไรต้องขอโทษเลย
    # (เคสนี้ category_detail ตกไปที่ "ชมรสชาติอาหาร" เพราะ keyword ไม่ match อะไร
    # เลยทั้งประโยค เลย fallback ไปเชื่อ category ที่ ML เดามาแบบไม่มั่นใจ)
    #
    # แก้โดยแยกกรณี neutral ออกมาต่างหาก ให้ตอบแบบเป็นกลาง (general_support) แทน
    # การขอโทษที่ไม่มีมูล — ไม่กระทบพฤติกรรมเดิมตอน positive/negative เลย
    # ---------- 5) รสชาติ (แยกตาม sentiment เพราะอาจเป็นได้ทั้งบวก/ลบ/กลาง) ----------
    if "รสชาติ" in c or "คุณภาพอาหาร" in c:
        if s == "positive":
            return "thank_you"
        if s == "negative":
            return "apology_escalate"
        return "general_support"  # neutral: ไม่ใช่คำชมหรือคำติที่ชัดเจน ไม่ควรขอโทษเกินจำเป็น

    # ---------------------------------------------------------------------
    # FIX (ข้อความจริงจากหน้าเพจ: "ณัฐกานต์ ปัจจุสนันท์ พาพี่มอสมากินสิ" — คอมเมนต์
    # แท็กเพื่อนชวนมากิน ไม่ได้ชมหรือติร้านเลย แต่ถูกตอบว่า "ขอบคุณมากค่ะ ทางร้านดีใจ
    # ที่ลูกค้าประทับใจ..." ทั้งที่ไม่มีใครพูดถึงความประทับใจอะไรเลย — เกิดจากคอมเมนต์
    # แบบนี้ไม่มี keyword ชม/ติตัวไหน match เลย เลยปล่อยให้ ML เดา sentiment เอง (เดา
    # เป็น positive เพราะโทนเป็นมิตร) แล้วไม่มี category ไหน match พอดี ตกไปจบที่กฎ
    # "positive ทั่วไป -> thank_you" ด้านล่าง)
    #
    # ตั้งใจวาง "ท้ายสุดก่อนกฎทั่วไป" ตรงนี้เท่านั้น (ไม่ใช่ไว้ต้นฟังก์ชัน) เพราะทดสอบ
    # แล้วพบว่าถ้าเช็คเร็วเกินไปจะไปแย่งคำตอบของคอมเมนต์ที่มีทั้งคำชวนกับคำชม/คำถาม
    # เจาะจงปนกันในประโยคเดียว (เช่น "พาแฟนมาทานมื้อนี้ อร่อยมากค่ะ" ควรได้รับคำขอบคุณ
    # เรื่องรสชาติ ไม่ใช่คำต้อนรับทั่วไป, "พาเพื่อนมากินเผ็ดมากไหมคะ" ควรได้ข้อมูลปรับ
    # ระดับความเผ็ด ไม่ใช่คำต้อนรับทั่วไป) วางไว้ตรงนี้เพื่อให้ทุก branch ที่เจาะจงกว่า
    # (หมวดข้อมูลต่างๆ ในข้อ 4 และรสชาติในข้อ 5) ได้ตัดสินใจก่อนเสมอ จะมาถึงจุดนี้ก็
    # ต่อเมื่อไม่มี category ไหนเจาะจงพอจริงๆ เท่านั้น (เข้าเงื่อนไขเดียวกับที่จะตกไป
    # "thank_you" ทั่วไปแบบเดิมพอดี จึงปลอดภัยที่จะแทนที่ตรงนี้)
    #
    # เช็คจากข้อความจริง (m) ตรงๆ ด้วย has_tag_friend_invite() (ดู comment เต็มใน
    # keywords_data.py — จงใจไม่ใช้ tokenizer เพราะชื่อเล่นคนที่ถูกแท็กมักทำให้ตัดคำ
    # ผิดเพี้ยน) กันไว้เฉพาะตอนไม่ใช่ negative (คำร้องเรียนจริงถูกกฎด้านบนจับไปตอบ
    # apology ตั้งแต่ก่อนถึงจุดนี้แล้ว)
    #
    # ---------------------------------------------------------------------
    # FIX (ผู้ใช้ท้วงต่อ: "ไม่ใช่แค่เรื่องชวนเพื่อนนะ ต้องตอบเรื่องอื่นๆได้ด้วยหรือไม่รู้
    # ว่านี้คือการแทคคน"): has_tag_friend_invite() ด้านบนจับได้แค่รูปแบบประโยคเฉพาะ
    # "พา/ชวน/แท็ก...มากิน/มาทาน" เท่านั้น ไม่ครอบคลุมการแท็กรูปแบบอื่น เพิ่มเงื่อนไข
    # has_facebook_tags(message_tags) เข้ามาด้วย (OR กับของเดิม) — เช็คจาก "ข้อเท็จจริง
    # ที่ Facebook ยืนยันมาว่ามีคนถูกแท็กจริง" (message_tags จาก Graph API ส่งผ่าน
    # Make.com -> API -> ตรงนี้) แทนการเดาจากคำพูด จึงครอบคลุมการแท็กทุกรูปแบบ ไม่ว่า
    # จะพิมพ์ว่าอะไร (แท็กเฉยๆไม่มีข้อความ, แท็กแล้วถามคำถาม, แท็กด้วยประโยคที่คิดไม่ถึง
    # ฯลฯ) ตราบใดที่ไม่มี category/sentiment ที่เจาะจงกว่าจับได้ก่อนถึงจุดนี้ (เช่น
    # คำชมรสชาติที่บังเอิญมีแท็กติดมาด้วย ยังคงได้ thank_you ตามปกติ เพราะ section 5
    # ด้านบนจะดักไปตอบก่อนถึงตรงนี้อยู่แล้ว — ดู test case "พาแฟนมาทานมื้อนี้ อร่อยมาก
    # ค่ะ" ที่ยืนยันแล้วว่าไม่ถูกแย่ง)
    #
    # has_tag_friend_invite(m) ยังคงไว้เป็น fallback สำรอง สำหรับตอนที่ Make.com ยัง
    # ไม่ได้ส่ง message_tags มา (เช่น ยังไม่ได้ตั้งค่า หรือ channel อื่นที่ไม่มีข้อมูลนี้)
    # ---------------------------------------------------------------------
    if s != "negative" and (has_facebook_tags(message_tags) or has_tag_friend_invite(m)):
        return "welcome_tagged_guest"

    # ---------- 6) positive ทั่วไป ----------
    if s == "positive":
        return "thank_you"

    if "ชมบรรยากาศร้าน" in c or "ชมพนักงาน" in c:
        return "thank_you"

    return "general_support"


REPLY_TEMPLATES = {
    "apology_coupon_escalate": "ขออภัยอย่างสูงค่ะ ทางร้านจะรีบตรวจสอบรายการนี้ให้ทันที และขอมอบคูปองส่วนลดสำหรับการสั่งครั้งถัดไปค่ะ",
    "apology_check_order": "ขออภัยในความไม่สะดวกค่ะ ทางร้านจะรีบตรวจสอบรายการอาหาร/การจัดส่งให้ทันทีค่ะ",
    "apology_wrong_order": "ขออภัยค่ะที่ออเดอร์ไม่ครบถ้วน ทางร้านจะตรวจสอบและจัดส่งของที่ขาดให้ทันทีค่ะ รบกวนแจ้งเลขที่ออเดอร์ด้วยนะคะ",
    "apology_quality_escalate": "ขออภัยอย่างสูงค่ะ เรื่องนี้เป็นเรื่องสำคัญมาก ทางร้านจะส่งต่อให้ทีมงานตรวจสอบด่วนที่สุด รบกวนขอรายละเอียดเพิ่มเติม (เช่น รูปภาพ) เพื่อให้เราปรับปรุงและดูแลคุณได้ดีขึ้นค่ะ",
    "apology_staff_escalate": "ขออภัยอย่างสูงค่ะสำหรับประสบการณ์ที่ไม่ดี ทางร้านจะนำเรื่องนี้ไปแจ้งและอบรมทีมงานเพื่อไม่ให้เกิดขึ้นอีกค่ะ",
    "apology_cleanliness_escalate": "ขออภัยค่ะ ทางร้านให้ความสำคัญกับความสะอาดเป็นอย่างมาก จะรีบตรวจสอบและปรับปรุงทันทีค่ะ",
    "apology_escalate": "ขออภัยอย่างสูงค่ะ ทางร้านรับทราบปัญหาแล้ว และจะส่งเรื่องให้ผู้จัดการตรวจสอบเพื่อปรับปรุงบริการค่ะ",
    "send_promotion": "{promotion_note} สนใจเมนูแนะนำเพิ่มเติมไหมคะ",
    "send_menu": "ยินดีค่ะ ทางร้านมีเมนูแนะนำหลายรายการ ({menu_note}) เดี๋ยวส่งรายละเอียดเมนูยอดนิยมให้ลูกค้าค่ะ",
    "reservation_support": "ยินดีค่ะ ลูกค้าต้องการจองโต๊ะวันและเวลาใด แจ้งจำนวนที่นั่งได้เลยค่ะ หรือโทรจองล่วงหน้าได้ที่ {phone} ค่ะ",
    "send_hours": "ทางร้านเปิดให้บริการ {opening_hours} ค่ะ ({closed_days}) หากมีวันหยุดพิเศษจะแจ้งให้ทราบล่วงหน้านะคะ",
    "send_location": "ร้านตั้งอยู่ที่ {address} ค่ะ ดูแผนที่ได้ที่ {map_link} ค่ะ มีที่จอดรถให้บริการ หากต้องการเส้นทางเพิ่มเติมแจ้งได้เลยค่ะ",
    "send_price_info": "รบกวนแจ้งเมนูที่สนใจ ทางร้านจะแจ้งราคาที่แน่นอนให้ค่ะ",
    "send_payment_info": "ทางร้านรับชำระได้หลายช่องทางค่ะ ได้แก่ {payment_methods}",
    "allergy_info_caution": "ขอบคุณที่แจ้งค่ะ รบกวนระบุอาการแพ้หรือข้อจำกัดด้านอาหารให้ชัดเจน ทางร้านจะแนะนำเมนูที่เหมาะสมและระวังส่วนผสมให้เป็นพิเศษค่ะ",
    "send_tax_invoice_info": "รบกวนแจ้งชื่อ-ที่อยู่ และเลขผู้เสียภาษีสำหรับออกใบกำกับภาษีเต็มรูปแบบนะคะ",
    "send_membership_info": "ทางร้านมีระบบสะสมแต้มค่ะ ทุกการสั่งซื้อสามารถสะสมแต้มแลกส่วนลด/ของรางวัลได้ สนใจสมัครสมาชิกไหมคะ",
    "catering_support": "ยินดีให้บริการจัดเลี้ยงค่ะ รบกวนแจ้งวันที่ จำนวนคน และงบประมาณโดยประมาณ ทางร้านจะจัดทำใบเสนอราคาให้ค่ะ",
    "job_application_info": "ขอบคุณที่สนใจร่วมงานกับเราค่ะ รบกวนแจ้งตำแหน่งที่สนใจ ทางร้านจะส่งรายละเอียดการสมัครให้ค่ะ",
    "refund_priority_support": "ขออภัยในความไม่สะดวกค่ะ ทางร้านจะดำเนินการคืนเงิน/ยกเลิกออเดอร์ให้โดยเร็วที่สุดเป็นกรณีพิเศษค่ะ",
    "refund_cancel_support": "รับทราบค่ะ ทางร้านจะดำเนินการคืนเงิน/ยกเลิกออเดอร์ให้ รบกวนแจ้งเลขที่ออเดอร์ด้วยนะคะ",
    "thank_you": "ขอบคุณมากค่ะ ทางร้านดีใจที่ลูกค้าประทับใจ หวังว่าจะได้ดูแลลูกค้าอีกครั้งนะคะ",
    "general_support": "ขอบคุณที่ติดต่อมาค่ะ ทางร้านยินดีให้บริการ ต้องการสอบถามข้อมูลเพิ่มเติมเรื่องใดแจ้งได้เลยค่ะ",
    "branch_location_info": "ขอบคุณที่สอบถามค่ะ ทางร้านมีหลายสาขา สามารถดูที่ตั้งและแผนที่แต่ละสาขาได้ทางลิงก์ด้านล่างนี้ค่ะ",
    "parking_info": "ร้านมีที่จอดรถให้บริการค่ะ หากที่จอดเต็มสามารถสอบถามพนักงานหน้าร้านเพื่อแนะนำจุดจอดใกล้เคียงได้เลยค่ะ",
    "apology_temperature_escalate": "ขออภัยในความไม่สบายค่ะ ทางร้านจะรีบตรวจสอบและปรับอุณหภูมิห้องให้เหมาะสมโดยเร็วที่สุดค่ะ",
    "apology_restroom_escalate": "ขออภัยค่ะ ทางร้านจะให้ทีมงานตรวจสอบและทำความสะอาดห้องน้ำทันทีค่ะ",
    "apology_billing_escalate": "ขออภัยค่ะสำหรับความผิดพลาดเรื่องยอดเงิน ทางร้านจะตรวจสอบบิลและแก้ไขให้ถูกต้องโดยเร็วที่สุดค่ะ รบกวนแจ้งเลขที่ออเดอร์ด้วยนะคะ",
    "apology_system_issue": "ขออภัยในความไม่สะดวกค่ะ ทางทีมงานกำลังเร่งแก้ไขปัญหาระบบ รบกวนลองใหม่อีกครั้งหรือแจ้งเจ้าหน้าที่โดยตรงได้เลยค่ะ",
    "apology_membership_issue": "ขออภัยค่ะ ทางร้านจะตรวจสอบข้อมูลบัตรสมาชิก/แต้มสะสมให้ รบกวนแจ้งเบอร์โทรหรือรหัสสมาชิกด้วยนะคะ",
    "waitlist_info": "ตอนนี้คิวประมาณ {queue_wait_estimate} ค่ะ ลูกค้าสามารถแจ้งชื่อ-เบอร์โทรไว้ล่วงหน้าเพื่อจองคิวได้เลยค่ะ",
    "wifi_info": "รหัส WiFi ของร้านคือ {wifi_code} ค่ะ สามารถสอบถามพนักงานหน้าร้านเพิ่มเติมได้เลยค่ะ",
    "pet_friendly_info": "ร้านอนุญาตให้พาสัตว์เลี้ยงเข้าได้ค่ะ (ตามพื้นที่ที่กำหนด) รบกวนพาน้องๆ อยู่ในสายจูงและดูแลตามมารยาทด้วยนะคะ",
    "kids_facility_info": "ร้านมีเก้าอี้เด็กและเมนูสำหรับเด็กให้บริการค่ะ สามารถแจ้งพนักงานเมื่อมาถึงร้านได้เลยค่ะ",
    "private_room_info": "ร้านมีห้องส่วนตัว/ห้อง VIP ให้บริการค่ะ รบกวนแจ้งจำนวนคนและวันเวลาที่สนใจใช้บริการนะคะ",
    "corkage_info": "ลูกค้าสามารถนำเครื่องดื่ม/เค้กมาเองได้ค่ะ (ค่า Corkage: {corkage_fee}) ค่ะ",
    "spice_customization_info": "ทางร้านสามารถปรับระดับความเผ็ดหรือรสชาติตามที่ลูกค้าต้องการได้ค่ะ รบกวนแจ้งรายละเอียดตอนสั่งได้เลยค่ะ",
    "buffet_info": "ร้านมีโปรบุฟเฟ่ต์ให้บริการค่ะ รบกวนแจ้งวันที่สนใจ ทางร้านจะแจ้งราคาและเงื่อนไขให้ค่ะ",
    "feedback_thank_you": "ขอบคุณมากค่ะสำหรับข้อเสนอแนะ ทางร้านจะนำไปปรับปรุงบริการให้ดียิ่งขึ้นค่ะ",
    "alcohol_menu_info": "ร้านมีเครื่องดื่มแอลกอฮอล์ให้บริการค่ะ สนใจดูเมนูเครื่องดื่มเพิ่มเติมไหมคะ",
    "event_info": "ร้านมีกิจกรรม/ดนตรีสดเป็นบางช่วงค่ะ รบกวนแจ้งวันที่สนใจ ทางร้านจะเช็คตารางให้นะคะ",
    "delivery_area_info": "รบกวนแจ้งที่อยู่หรือพื้นที่ที่ต้องการจัดส่ง ทางร้านจะเช็คว่าอยู่ในพื้นที่บริการหรือไม่ค่ะ",
    "min_order_info": "ยอดสั่งขั้นต่ำสำหรับบริการเดลิเวอรี่คือ {min_delivery_order} ค่ะ",
    "franchise_info": "ขอบคุณที่สนใจแฟรนไชส์ของเราค่ะ รบกวนแจ้งชื่อ-เบอร์ติดต่อ ทางร้านจะให้ทีมงานส่งรายละเอียดการลงทุนให้ค่ะ",
    "contact_info": "ติดต่อร้านได้ทางเบอร์โทร {phone} หรือ LINE {line_id} ค่ะ ทางร้านยินดีให้บริการค่ะ",
    "takeaway_info": "สามารถห่อกลับได้ค่ะ แจ้งพนักงานหน้าร้านหรือระบุตอนสั่งได้เลยค่ะ",
    "delivery_info": "สามารถสั่งผ่านบริการเดลิเวอรี่ได้ค่ะ รบกวนแจ้งแพลตฟอร์ม/แอปที่สนใจใช้ ทางร้านจะเช็คโปรโมชั่นและเงื่อนไขที่ใช้ได้กับเดลิเวอรี่ให้นะคะ",
    "welcome_tagged_guest": "ขอบคุณที่แท็กเพื่อนมาเที่ยวร้านด้วยกันนะคะ ยินดีต้อนรับทุกท่านเลยค่ะ",
    "price_feedback_ack": "ขอบคุณสำหรับความคิดเห็นเรื่องราคาค่ะ ทางร้านขอรับข้อเสนอแนะไว้พิจารณาปรับปรุงต่อไป หากมีข้อสงสัยเรื่องราคาเมนูใดเพิ่มเติมสอบถามได้เลยนะคะ",
}

def make_reply(action):
    """
    หยิบคำตอบต้นแบบตาม action แล้วแทนที่ placeholder เช่น {wifi_code}/{opening_hours}
    ด้วยข้อมูลจริงของร้านจาก shop_config.get_shop_info() (ดู shop_config.py — เจ้าของ
    ร้านแก้ไขค่าพวกนี้เองได้โดยไม่ต้องแตะไฟล์นี้เลย) ครอบด้วย try/except กันเหนียว
    เผื่อ template มี placeholder ที่ไม่มีอยู่ใน shop_info (ไม่ควรเกิดขึ้นถ้าตั้งชื่อ
    key ตรงกัน) จะได้ไม่ทำให้ /make/predict พังเพราะเรื่องนี้เด็ดขาด
    """
    template = REPLY_TEMPLATES.get(action, REPLY_TEMPLATES["general_support"])
    try:
        return template.format(**get_shop_info())
    except Exception:
        return template
