# -*- coding: utf-8 -*-
from app.shop_config import get_shop_info
...
def choose_action(sentiment, category, segment, message=""):
    ...
    # FIX (เคสร้ายแรงที่สุดจากการ audit หน้าเพจจริง: "เจอเส้นผมในอาหารค่ะ ขอให้
    # ตรวจสอบด้วย" เดิมไม่มี "เส้นผม" อยู่ใน list นี้เลย ทำให้ไม่เคยเข้า branch escalate
    # พิเศษนี้แม้ sentiment จะถูกแก้ให้เป็น negative แล้วก็ตาม (ดู TASTE_NEG ใน
    # keywords_data.py ที่เพิ่ม "เส้นผม" เข้าไปด้วยเพื่อให้ keyword ตัดสิน sentiment
    # เป็น negative ได้เองโดยไม่ต้องพึ่ง ML) เพิ่มไว้ตรงนี้เพื่อให้ตอบ apology_quality_
    # escalate (escalate ทุกเซกเมนต์) แทน apology_escalate ธรรมดา เพราะสิ่งแปลกปลอม
    # ในอาหารเป็นเรื่องร้ายแรงกว่าคำร้องเรียนทั่วไปมาก
    if s == "negative" and any(w in m for w in ["แมลง", "สิ่งแปลกปลอม", "อาหารเป็นพิษ", "บูด", "เสียแล้ว", "หมดสภาพ", "เส้นผม"]):
    ...
    # ---------------------------------------------------------------------
    # FIX (เคสจริง "โปรลดราคานี้ใช้ได้กับเดลิเวอรี่ด้วยไหมคะ" ถูกตอบ general_support
    # ทั้งที่ category ตัดสินถูกแล้วว่าเป็น "การจัดส่ง (Delivery)"): เดิม category นี้
    # มีแค่ branch ร้องเรียน (ด้านบน, เช็คเฉพาะตอน s=="negative") ไม่มี branch ตอบ
    # ข้อมูลทั่วไปสำหรับคำถามที่เป็นกลางเกี่ยวกับเดลิเวอรี่เลย (ต่างจากหมวดอื่นๆ เช่น
    # "เมนู"/"โปรโมชั่น" ที่มี branch ข้อมูลทั่วไปของตัวเองในส่วนที่ 4) ทำให้ตกไปจบที่
    # general_support เสมอเมื่อไม่ใช่การร้องเรียน เพิ่ม branch นี้ไว้ตรงนี้ (หลัง branch
    # ร้องเรียนข้างบนเสมอ เพื่อให้คำร้องเรียนจริงยังคงได้รับการ apology ก่อน ไม่ถูกบัง)
    #
    # จำกัดเฉพาะ s=="neutral" เท่านั้น (ไม่ใช่ s!="negative" ตามที่ลองตอนแรก) เพราะ
    # เทสกับ regression suite เดิมพบว่า "แพ็คมาดีมาก ของไม่หกเลย" (คำชมเดลิเวอรี่ที่
    # เป็น positive อยู่แล้ว) จะโดน branch นี้แย่งไปตอบ "delivery_info" แทน "thank_you"
    # ที่ถูกต้องอยู่แล้ว ถ้าเช็คแค่ s!="negative" — คำชมเดลิเวอรี่ยังต้องได้ thank_you
    # เหมือนเดิมทุกประการ (ดู "# ---------- 6) positive ทั่วไป ----------" ด้านล่าง)
    # ---------------------------------------------------------------------
    if s == "neutral" and ("การจัดส่ง" in c or "Delivery" in c):
    ...
    # ---------------------------------------------------------------------
    # FIX (ข้อความจริง "ขอชมว่าน้ำแข็งเย็นทุกก้อน" ถูกตอบเป็นข้อมูลที่ตั้งร้าน):
    # category_model (ML) มี class ดิบที่ขึ้นต้นด้วย "ชม..." อยู่ 3 ตัว คือ
    # "ชมรสชาติอาหาร", "ชมการบริการ", "ชมสถานที่/บรรยากาศ" (ดู models/category_model.joblib)
    # ปัญหาคือ "ชมสถานที่/บรรยากาศ" ดันมีคำว่า "สถานที่" อยู่ในตัวเอง ซึ่งไปชนกับกฎข้อ 4
    # ด้านล่างที่เช็ค "สถานที่" in c -> send_location (กฎนั้นตั้งใจไว้จับ category
    # "ที่ตั้งร้าน (Location)" ของลูกค้าที่ถาม ไม่ใช่ category คำชมบรรยากาศ) ทำให้คำชม
    # กลายเป็นถูกส่งพิกัดร้านให้แทนคำขอบคุณ จึงกันไว้ก่อนตรงนี้: ถ้า sentiment เป็น
    # positive และ category ขึ้นต้นด้วย "ชม" (ไม่ว่าจะเป็น label จาก ML หรือจาก
    # keyword เอง เช่น "ชมบรรยากาศร้าน"/"ชมพนักงาน"/"ชมความสะอาด") ให้ตอบขอบคุณทันที
    # ก่อนไปถึงกฎ "สอบถามข้อมูลทั่วไป" ด้านล่าง ไม่ต้องรอไปชนคำบังเอิญ
    # ---------------------------------------------------------------------
    if s == "positive" and c.startswith("ชม"):
        return "thank_you"
    ...
    # FIX (เคสจริง "โดยรวมโอเค แต่ราคาแพงขึ้นเยอะเลย" ถูกตอบ "รบกวนแจ้งเมนูที่สนใจ
    # ทางร้านจะแจ้งราคาที่แน่นอนให้ค่ะ" ทั้งที่ลูกค้ากำลังบ่นเรื่องราคาขึ้น ไม่ได้ถามราคา):
    # ต้องเช็ค "ร้องเรียนราคา" (คำร้องเรียน) แยกจาก "ราคา" ทั่วไป (คำถามราคาเฉยๆ) ก่อน
    # เพราะ "ร้องเรียนราคา" ก็มีคำว่า "ราคา" เป็น substring อยู่ในตัวเองด้วย ถ้าไม่แยก
    # เช็คก่อน จะโดน branch "ราคา" in c ด้านล่างตัดหน้าเสมอ (sentiment เคสนี้มักออกมา
    # เป็น "neutral" จาก majority-vote แบบก้ำกึ่ง ไม่ใช่ "negative" ล้วนๆ เพราะมักมีคำชม
    # ปนอยู่ด้วยเช่น "โดยรวมโอเค" จึงไม่เข้า branch ร้องเรียนหนักด้านบนที่ต้อง s=="negative")
    if "ร้องเรียนราคา" in c:
        return "price_feedback_ack"
...
REPLY_TEMPLATES = {
    ...
    "send_promotion": "{promotion_note} สนใจเมนูแนะนำเพิ่มเติมไหมคะ",
    "send_menu": "ยินดีค่ะ ทางร้านมีเมนูแนะนำหลายรายการ ({menu_note}) เดี๋ยวส่งรายละเอียดเมนูยอดนิยมให้ลูกค้าค่ะ",
    "reservation_support": "ยินดีค่ะ ลูกค้าต้องการจองโต๊ะวันและเวลาใด แจ้งจำนวนที่นั่งได้เลยค่ะ หรือโทรจองล่วงหน้าได้ที่ {phone} ค่ะ",
    "send_hours": "ทางร้านเปิดให้บริการ {opening_hours} ค่ะ ({closed_days}) หากมีวันหยุดพิเศษจะแจ้งให้ทราบล่วงหน้านะคะ",
    "send_location": "ร้านตั้งอยู่ที่ {address} ค่ะ ดูแผนที่ได้ที่ {map_link} ค่ะ มีที่จอดรถให้บริการ หากต้องการเส้นทางเพิ่มเติมแจ้งได้เลยค่ะ",
    "send_price_info": "รบกวนแจ้งเมนูที่สนใจ ทางร้านจะแจ้งราคาที่แน่นอนให้ค่ะ",
    "send_payment_info": "ทางร้านรับชำระได้หลายช่องทางค่ะ ได้แก่ {payment_methods}",
    ...
    "waitlist_info": "ตอนนี้คิวประมาณ {queue_wait_estimate} ค่ะ ลูกค้าสามารถแจ้งชื่อ-เบอร์โทรไว้ล่วงหน้าเพื่อจองคิวได้เลยค่ะ",
    "wifi_info": "รหัส WiFi ของร้านคือ {wifi_code} ค่ะ สามารถสอบถามพนักงานหน้าร้านเพิ่มเติมได้เลยค่ะ",
    ...
    "corkage_info": "ลูกค้าสามารถนำเครื่องดื่ม/เค้กมาเองได้ค่ะ (ค่า Corkage: {corkage_fee}) ค่ะ",
    ...
    "min_order_info": "ยอดสั่งขั้นต่ำสำหรับบริการเดลิเวอรี่คือ {min_delivery_order} ค่ะ",
    "franchise_info": "ขอบคุณที่สนใจแฟรนไชส์ของเราค่ะ รบกวนแจ้งชื่อ-เบอร์ติดต่อ ทางร้านจะให้ทีมงานส่งรายละเอียดการลงทุนให้ค่ะ",
    "contact_info": "ติดต่อร้านได้ทางเบอร์โทร {phone} หรือ LINE {line_id} ค่ะ ทางร้านยินดีให้บริการค่ะ",
    "takeaway_info": "สามารถห่อกลับได้ค่ะ แจ้งพนักงานหน้าร้านหรือระบุตอนสั่งได้เลยค่ะ",
    "delivery_info": "สามารถสั่งผ่านบริการเดลิเวอรี่ได้ค่ะ รบกวนแจ้งแพลตฟอร์ม/แอปที่สนใจใช้ ทางร้านจะเช็คโปรโมชั่นและเงื่อนไขที่ใช้ได้กับเดลิเวอรี่ให้นะคะ",
    "price_feedback_ack": "ขอบคุณสำหรับความคิดเห็นเรื่องราคาค่ะ ทางร้านขอรับข้อเสนอแนะไว้พิจารณาปรับปรุงต่อไป หากมีข้อสงสัยเรื่องราคาเมนูใดเพิ่มเติมสอบถามได้เลยนะคะ",
}

def make_reply(action):
    """
    หยิบคำตอบต้นแบบตาม action แล้วแทนที่ placeholder เช่น {wifi_code}/{opening_hours}
    ด้วยข้อมูลจริงของร้านจาก shop_config.get_shop_info() (ดู shop_config.py — เจ้าของ
    ร้านแก้ไขค่าพวกนี้เองได้โดยไม่ต้องแตะไฟล์นี้เลย) ครอบด้วย try/except กันเหนียว
    เผื่อ template มี placeholder ที่ไม่มีอยู่ใน shop_info (ไม่ควรเกิดขึ้นถ้าตั้งชื่อ
    key ตรงกัน) จะได้ไม่ทำให้ /make/predict พังเพราะเรื่องนี้เด็ดขาด
    """
    template = REPLY_TEMPLATES.get(action, REPLY_TEMPLATES["general_support"])
    try:
        return template.format(**get_shop_info())
    except Exception:
        return template
